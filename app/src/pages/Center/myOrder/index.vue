              ...
              <table class="order-item" v-for="(order,index) in myOrder.records" :key="order.id">
                ...
                <tbody>
                  <tr v-for="(cart,index) in order.orderDetailList" :key="cart.id">
                    ...
                    <!-- v-if="index==0"表示后面的收件人以及相关的信息只需要一份，其他的不现实 -->
                    <!-- :rowspan="order.orderDetailList.length" 表示前面的订单数需要根据数据的多少来进行表单的合并 -->
...
export default {
  ...
  mounted(){
    this.getData();   //获取我的订单的数据的方法
  },
  methods:{
   async getData(){   //获取我的订单的方法
    //解构出参数
