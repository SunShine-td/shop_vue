...
import throttle from "lodash/throttle"
  export default {
      ...
      handler:throttle(async function(type,disNum,cart){
        ...
        switch(type){
          ...
          case "add":
            ...
            break;
          case "minus":
            //判断产品的个数大于1，才可以传递服务器-1
            //如果出现产品的个数小于等于1，传递给服务器个数为0(原封不动)
            disNum=cart.skuNum>1?-1:0;
            break;
          case "change":
            //用户输入进来的最终量，如果非法的(带有汉字||出现负数) 带给服务器的数字为0，原封不动
            if(isNaN(disNum)||disNum<1){
              disNum=0;
            }else{    //用户输入正常的情况(小数：取整)，带给服务器变化的量 用户输入进来的-产品起始的个数
    ...
    computed:{
      ...
      isAllCheck(){
        //遍历数组里面元素，主要全部元素isChecked属性全为1===>真 true
        //只要有一个不是1======>假  false
