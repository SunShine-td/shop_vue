...
路由传递参数先关面试题
     ...
     不可以：不能这样书写，程序会崩掉 （路由跳转传参的时候，对象的写法可以是name path形式，但是需要注意的是，path的这种写法不能与params参数一起使用）
     2:如何指定params参数可传可不传? 

​			*如果路由器要求传递params参数，但是你就不传递params参数，发现url会出问题

​			*如何指定params参数可以传递或者不传递，在路由配置的时候，在占位符的后面加一个问号【params可以传递也可以不传递】
...
​     5: 路由组件能不能传递props数据?	可以传递props数据
​     	
