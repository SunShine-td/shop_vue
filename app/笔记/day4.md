...
第三步:准备模拟的数据(在mock文件夹下创建相应的JSON文件) 记得格式化文档一下，不要留有空格
把mock数据需要的图片放置于public文件夹中！【public文件夹再把打包的时候，会把相应的资源原封不动的打包到dist文件夹中】
...
第四步：开始mock，通过mockjs模块实现

在mock文件夹中创建一个mockserver.js文件,通过mockjs插件实现模拟数据
