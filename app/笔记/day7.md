...
为什么许多项目采用分页功能，比如电商平台同时展示的数据有很多

element-Ui是有相应的分页组件的，使用起来超级简单，但是咱们前台项目目前不用【掌握自定义分页器功能】

6.1 分页器展示，需要哪些数据（条件）？自定义分页器需要知道的四个前提条件
...
6.2 自定义分页器，在开发的时候先自己用数据进行测试模拟，调试成功后再用服务器数据

6.3）对于分页器而言，很重要的一个地方即为【算出：连续页面起始数字和结束数字】****
